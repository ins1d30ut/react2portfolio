...
import {
  MoneyButtonClient,
  AuthError as MoneyButtonAuthError,
} from '@moneybutton/api-client';
...
const LoginButton = () => <button onClick={loginMB}>Login</button>;

const LoggedIn = () => <button className='nametarget'>User:</button>;
...
export default class Oauth extends React.Component {
  ...
  async componentDidMount() {
    try {
      ...
      const { id: moneyButtonId, name: moneyButtonName } =
    ...
    if (this.state.loggedIn) {
      document.querySelector('.nametarget').innerText +=
  ...
  render() {
    return (
      <section>{this.state.loggedIn ? <LoggedIn /> : <LoginButton />}</section>
    );
  }
}
